...
class NeuralNetwork:
    def __init__(self, state_size, action_size):
        ...
        self.file_path = os.path.dirname(os.path.realpath(__file__)) + "/saved_model/" + "model_"
        #self.file_path = self.file_path.replace('turtlebot2i_deep_qlearning/dqn', 'turtlebot2i_deep_qlearning/save_model/model_')

        # make false if you want to start training from scratch
        ...
        if self.load_model:
            self.model.set_weights(load_model(self.file_path + str(self.load_episode) + ".h5").get_weights())

            with open(self.file_path + str(self.load_episode) + '.json') as outfile:
